...
const allActive = async (req, res) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    resObj.recordId = line.recordId;
    resObj.title = line.title;
    resObj.applicationGroup = line.applicationGroup;
    resObj.applicationGroupName = applicationGroupName;
    resObj.createdBy = line.createdBy;
    resObj.createdByName = createdByName;
    resObj.createAt = line.created_at;
    resObj.commentCount = commentCount;
    resObj.isUnConfirmed = isUnConfirmed;
    resObj.thumbNailItemId = thumbNailItemId;
    resObj.updatedAt = line.updatedAt;
...
const mineActive = async (req, res) => {
  ...
  const searchRecordQs = `select record_id, created_by, application_group, updated_at from record where created_by = ? and status = "open" order by updated_at desc, record_id asc limit ? offset ?`;
